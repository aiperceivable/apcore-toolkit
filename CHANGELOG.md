...
The first release since 0.11.1, and a substantial one: it adds the toolkit's first credential-handling surface (**RFC 8628 Device Authorization Flow**, [#17](https://github.com/aiperceivable/apcore-toolkit/issues/17)), closes a latent `BindingLoader` spec gap ([#18](https://github.com/aiperceivable/apcore-toolkit/issues/18)), fixes a credential-disclosure path in `$ref` resolution, and raises the apcore floor to 0.31.0.

The device-auth half is the protocol only — the toolkit writes nothing to a terminal; it emits events and the consumer renders them.

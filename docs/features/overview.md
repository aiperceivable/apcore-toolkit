...
| `HTTPProxyRegistryWriter` | Python + Rust (feature `http-proxy`) | Rust is feature-gated to keep the default build free of an HTTP-client dependency. TypeScript does not ship an HTTP proxy writer. |
